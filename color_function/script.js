...
let size = (Math.abs(size_lower) + size_upper) / pixel_size;
let absolute_width
...
class pixel {
  ...
  hue_changed(x, y) {
    var hue_expression = get_hue_expression.value
      .replace(/X/g, x)
      .replace(/Y/g, y);
  ...
  saturation_changed(x, y) {
    var saturation_expression = get_saturation_expression.value
      .replace(/X/g, x)
      .replace(/Y/g, y);
    this.saturation = Math.abs(
      ((100 + Function(`return ${saturation_expression}`)()) % 200) - 100
    );
    this.tegn();
  }
  lightness_changed(x, y) {
    var lightness_expression = get_lightness_expression.value
      .replace(/X/g, x)
      .replace(/Y/g, y);
    this.lightness = Math.abs(
      ((100 + Function(`return ${lightness_expression}`)()) % 200) - 100
    );
    this.tegn();
  }
}
  ...
  switch (true) {
    case new_size > size_upper:
      ...
      ctx.drawImage(
        ...
        0,
        0,
        ((600 / size) * (size - (new_size - old_size_upper))).toFixed(4),
        ((600 / size) * (size - (new_size - old_size_upper))).toFixed(4)
      );
  ...
  switch (true) {
    case new_size < size_lower:
      ...
      ctx.drawImage(
        ...
        (600 / size) * (old_size_lower - new_size),
        ((600 / size) * (size - (old_size_lower - new_size))).toFixed(4),
        ((600 / size) * (size - (old_size_lower - new_size))).toFixed(4)
      );
...
function change_pixel_size() {
  ...
  create_pixels(size_lower, size_upper);
}
...
function get_cursor_position(canvas, event) {
  ...
  // var absolute_heigth_pixel = canvas.height / size;
...
function zoom_guider() {
  ...
  var height;  //Positive values down, negative values up.
  var width;   //positive values to the right, negative to the left.
...
function new_pixels(dimension_start_x,
  dimension_start_y,
  dimension_width,
  dimension_length
) {
  ...
  for (let x = dimension_start_x, width = dimension_width; x < width; x++) {
    if (matrix_pixels[x] == undefined) {
      matrix_pixels[x] = new Array(~~dimension_length);
    }

    for (
      let y = dimension_start_y, length = dimension_length;
      y < length;
      y++
    ) {
  ...
  if (
    dimension_start_x == dimension_start_y &&
    dimension_width == dimension_length
  ) {
    dataURL = canvas.toDataURL();
    resizing_img.src = dataURL;
    original_img.src = dataURL;
  ...
  for (let x = dimension_start_y; x < dimension_length; x++) {
    if (matrix_pixels[x] == undefined) {
      matrix_pixels[x] = [];
      // matrix_pixels[x] = new Array(~~(dimension_width));
    }
